    ...
    <div className="font-nunito items-center flex flex-col p-6 md:p-20 md:bg-[url('/pages/share.png')] md:bg-cover md:bg-center bg-no-repeat">
      ...
      <div className="flex flex-col lg:flex-row w-full md:w-[90%] lg:w-[80%] gap-4">
        {data.cards.map((card, index) => (
          <div
            className="bg-card-bg w-full lg:w-[50%] p-6 md:p-8 flex flex-col shadow-[0_2px_8px_0_rgba(0,0,0,0.35)] rounded-[10px] justify-between"
            key={index}
          >
            <div className="flex flex-col items-start">
              {index === 0 ? (
                <FaRegClipboard className="text-primary h-16 w-16 md:h-20 md:w-20 mb-4 shrink-0" />
              ) : (
                <MdOutlineChat className="text-primary h-16 w-16 md:h-20 md:w-20 mb-4 shrink-0" />
              )}
            ...
            <Link
              href={card.href}
              className={`cursor-pointer mt-10 grid grid-cols-[30px_1fr_20px] sm:grid-cols-[40px_1fr_24px] items-center w-full rounded-[10px] p-3 transition-all duration-200 ${
                index === 0
            ...
            >
              {index === 0 ? (
                <SlNote className="h-7 w-7 justify-self-start" />
              ) : (
                <MdOutlineChat className="h-7 w-7 justify-self-start" />
              )}
              ...
              <FaArrowRight className="h-5 w-5 justify-self-end" />
            </Link>
          </div>
        ))}
      </div>
